...
#define heightTop 1000
#define heightTestMode 0
#define heightTestModeTop 500
#define heightTestModeH1 1500
#define heightTestModeH2 500

#define NegativeTolerance -5 
#define PositiveTolerance 5
...
void fuzzyGraph( fGraph *gX, IplImage *graph ){
	...
	if( gX -> counter == 300 || gX -> counter == 0){
		if( gX -> counter == 300 ){
			gX->counter2++;
			sprintf( filename, "graph%d.BMP", gX->counter2 );
